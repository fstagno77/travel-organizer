...
(function() {
  ...
  function renderTrains(container, trains) {
    ...
    const quickUploadCard = `
      <div class="quick-upload-card" id="quick-upload-trains">
    ...
    const sectionHeader = `
      <div class="section-header">
        ...
        <div class="section-header-actions">
          <button class="section-header-cta btn btn-primary" id="trains-add-booking-btn">
            ${plusSvg}
    ...
    i18n.apply(container);
    window.tripPage.initQuickUploadCard('quick-upload-trains');

    const addBtn = document.getElementById('trains-add-booking-btn');
    if (addBtn) {
      addBtn.addEventListener('click', () => {
        const tripId = window.tripPage.currentTripData?.id;
        if (tripId) window.tripPage.showAddBookingModal(tripId, 'train');
      });
    }
